...
nus = np.array([10**20, 10**20.5, 10**21])/pl.params.mantle.rho
        ...
        for X_Si_0 in X_Sis:
            for T_cmb0 in T_cmbs:
                ...
                try :
                    ...
                    filepath = '../computed_solutions_new/Tc{:d}_WtMg{:.2f}_WtSi{:.2f}_WtO{:.2f}_fMb{:.2f}_Xmb{:.2f}_XSb{:.2f}_nu{:.1e}_lthck{:.0e}_ovt{:.0e}/'.format(T_cmb0,
                            wpms,wpss,wpos, fraction_MgFe_b, X_MgFeO_b, X_SiO2_b, nu_present,layer_thickness,overturn)
                    ...
                    mplt.composition_wt(pl, times, solution, filepath=filepath)
                    mplt.dTdt(pl, times, solution, filepath=filepath)
                    mplt.MgSiOequilibrium(pl, times, solution, filepath=filepath)
                    mplt.MgFefraction(pl, times, solution, filepath=filepath)
                    mplt.K_vals(pl, times, solution,filepath=filepath)
                    t,all_parameters = pl.core_layer.compute_all_parameters(times, solution)
                    mplt.Q_all(pl, t, all_parameters,filepath=filepath)
                    mplt.E_all(pl, t, all_parameters,filepath=filepath)
                    plt.close('all')
                    time = str(datetime.datetime.now())
                    r_i = pl.core_layer.r_i(solution[-1,0], one_off=True)
                    del pl
                    csvdata = [time, T_cmb0, T_um0, r_i, wpms,wpss,wpos, fraction_MgFe_b, X_MgFeO_b, X_SiO2_b, nu_present,layer_thickness,overturn,X_Mg_0, X_Si_0, X_O_0]
                    ...
                    print("initial core composition invalid, no mantle equilibrium composition possible.")
                    del pl
                    # time = str(datetime.datetime.now())
                    # r_i = np.nan
                    ...
                    print("dTdt should not be >0., something is wrong.")
                    del pl
                except:
